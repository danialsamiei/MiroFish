...
优化改进：
1. 调用GraphitiClient检索功能二次丰富节点信息
2. 优化提示词生成非常详细的人设
3. 区分个人实体和抽象群体实体
"""
...
class OasisProfileGenerator:
    ...
    优化特性：
    1. 调用GraphitiClient图谱检索功能获取更丰富的上下文
    ...
    def __init__(
        ...
        # GraphitiClient for graph search
        ...
        if Config.NEO4J_PASSWORD:
            try:
                ...
                logger.warning(f"GraphitiClient初始化失败: {e}")
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        使用GraphitiClient混合搜索功能获取实体相关的丰富信息
        ...
        def search_edges():
            ...
            for attempt in range(max_retries):
                try:
                    return self.graph_client.search(
                        self.graph_id,
                        comprehensive_query,
                        30,
                        scope='edges'
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"边搜索第 {attempt + 1} 次失败: {str(e)[:80]}, 重试中...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"边搜索在 {max_retries} 次尝试后仍失败: {e}")
            return None
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"节点搜索第 {attempt + 1} 次失败: {str(e)[:80]}, 重试中...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"节点搜索在 {max_retries} 次尝试后仍失败: {e}")
            return None
        ...
        try:
            ...
            logger.warning(f"图谱检索超时 ({entity_name})")
        except Exception as e:

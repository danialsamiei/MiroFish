...
from .graph_client import NodeInfo as GNodeInfo, EdgeInfo as GEdgeInfo, SearchResult as GSearchResult
...
class ZepToolsService:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"GraphitiClient {operation_name} 第 {attempt + 1} 次尝试失败: {str(e)[:100]}, "
                        f"{delay:.1f}秒后重试..."
                    )
                    time.sleep(delay)
                    delay *= 2
                else:
                    logger.error(f"GraphitiClient {operation_name} 在 {max_retries} 次尝试后仍失败: {str(e)}")
    ...
    def search_graph(
        ...
        如果GraphitiClient的search不可用，则降级为本地关键词匹配。
        ...
        try:
            ...
            # 同时包含GraphitiClient返回的facts（可能包含额外的事实）
            ...
            logger.warning(f"GraphitiClient Search失败，降级为本地搜索: {str(e)}")
            # 降级：使用本地关键词匹配搜索
    ...
    def _local_search(
        ...
        """
        本地关键词匹配搜索（作为GraphitiClient Search的降级方案）
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        g_nodes = self.client.get_all_nodes(graph_id)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        g_edges = self.client.get_all_edges(graph_id, include_temporal=include_temporal)
